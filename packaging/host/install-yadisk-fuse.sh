...
if ! command -v pkg-config >/dev/null 2>&1; then
  if (command -v pkexec >/dev/null 2>&1 || command -v sudo >/dev/null 2>&1) && command -v apt-get >/dev/null 2>&1; then
    echo "[install] pkg-config is missing; installing it"
    run_privileged apt-get update
    run_privileged apt-get install -y pkg-config
  else
...
if ! pkg-config --exists fuse3; then
  if (command -v pkexec >/dev/null 2>&1 || command -v sudo >/dev/null 2>&1) && command -v apt-get >/dev/null 2>&1; then
    ...
    run_privileged apt-get update
    run_privileged apt-get install -y libfuse3-dev
  else
